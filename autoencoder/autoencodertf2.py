...
class Autoencoder:
    ...
    def __init__(self,
        ...
        self.decoder = None
        self.model = None
        self.base_folder = os.path.join(output_folder,name + "_date-" + datetime.now().strftime("%Y-%m-%d-%H-%M-%S"))
        self._num_conv_layers = len(conv_filters)
        self._shape_before_bottleneck = None
        self._model_input = None
        self._build()
        #
        # logger_file = os.path.join(self.base_folder,'log.log')
    ...
    def compile(self, learning_rate=0.0001):
        ...
        # metricsi = [tf.keras.metrics.MeanSquaredError,
        #            tf.keras.metrics.MeanAbsoluteError]
        # metricsi = [metrics.mean_squared_error,
    ...
    def save(self, save_folder=None):
        ...
        fig_name = os.path.join(save_folder, 'performance.png')
        self.plot_performance(fig_name)

    def plot_performance(self, fig_name):
        metric = 'loss'
...
class DeepAutoencoder:
    ...
    def _build_encoder(self):
        ...
        self._model_input = encoder_input
        encoded = Dense(self.latent_space_dim * 4, activation='relu')(encoder_input)
        encoded = Dense(self.latent_space_dim * 2, activation='relu')(encoded)
        encoded = Dense(self.latent_space_dim, activation='relu')(encoded)
    ...
    def _build_decoder(self):
        ...
        decoded = Dense(self.latent_space_dim * 2, activation='relu')(decoder_input)
        decoded = Dense(self.latent_space_dim * 4, activation='relu')(decoded)
    ...
    def train(self, x_train, batch_size, num_epochs):
        # autoencoder.fit(x_train, x_train, epochs=my_epochs, batch_size=256, shuffle=True,
    ...
    def load(cls, save_folder="."):
        ...
        with open(parameters_path, "rb") as f:
            parameters = pickle.load(f)
        autoencoder = DeepAutoencoder(*parameters)
        weights_path = os.path.join(save_folder, "weights.h5")
        autoencoder.load_weights(weights_path)
        return autoencoder
